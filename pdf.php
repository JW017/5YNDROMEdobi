...
//$ID=1;
  ...
  $sql="SELECT invoices.InvNo, users.Name, users.Address, invoices.SalesDate FROM invoices JOIN UserID ON users.UserID=invoices.UserID WHERE invoices.InvNo='$ID'";

  $res=$conn->query($sql);
  if($res->num_rows>0){
	  $row=$res->fetch_assoc();
	  
	  $obj=new Currency($row["GRAND_TOTAL"]);
	 
	  $info=[
		"customer"=>$row["Name"],
		"address"=>$row["Address"],
		"invoice_no"=>$row["InvNo"],
		"invoice_date"=>date("d-m-Y",strtotime($row["SalesDate"])),
  ...
  $sql="SELECT invoices.InvNo, items.ItemName, items.ItemPrice, invdetails.Qty FROM invdetails JOIN items ON items.ItemID=invdetails.ItemID JOIN invoices ON invoices.InvNo=invdetails.invNo WHERE invoices.InvNo='$ID'";
  $res=$conn->query($sql);
  if($res->num_rows>0){
